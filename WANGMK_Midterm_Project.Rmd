...
$$(1-1.828B+0.842B^{2})[(1-B)Y_{n}]=(1+0.941B)\epsilon_{n}$$
 in which $\{\epsilon_{n}\}$ is a series of normal white noise with  a variance of $5.62\times 10^{-8}$.
...
The modulus of the two AR component roots are `r round(ar_root_mod[1], digits=3)` and `r round(ar_root_mod[2], digits=3)` and the modulus of the MA component root is `r round(ma_root_mod, digits=3)`. Since all the roots fall outside the circle, I can say that this is a causal and invertible model. 
...
I also check the autocorrelation of the residuals from the model

```{r}
model_acf =acf(model$residuals, lag.max=80, main="Residuals of ARMA(2,1,1) Model")
```

I notice that there are significant correlations at lag 18, 36, 54 and 72. This phenomenon indicates that adding a SARMA term of lag 18 could possibly help. Given that the sign of correlation is different for lag 18 and lag 36, I would like to try a $SARIMA(2,1,1)\times (2,0,0)$. The model specification is
...
The AR component has two roots with modulus `r round(ar_mod[1], digits=3)` and `r round(ar_mod[2], digits=3)`. The MA component has two roots with modulus `r round(ma_mod, digits=3)`. The SAR component has two roots with modulus `r round(sar_mod[1], digits=3)` and `r round(sar_mod[2], digits=3)`. One of the AR roots is within the unit circle which makes the causality presumption a little problematic, but all the other roots are outside the unit circle, so the invertibility is preserved.


In the autocorrelation plot, we can see that the autocorrelation at lag 18, 36, 54 and 72 are reduced by some amount, but lag 36 and 54 are still significant. Overall this is not a big improvement. It seems that we need some more delicate techniques to deal with the seasonality, but I am running out of time to try them.
